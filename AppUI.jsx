...
// 🚨 IMPORTACIONES DEL MOTOR DE VIDEO 🚨
...
export default function AppUI() {
  ...
  // --- ESTADO DEL ESTUDIO DE VIDEO ---
  const [videoFiles, setVideoFiles] = useState([]);
  const [isRendering, setIsRendering] = useState(false);
  const [ffmpegLog, setFfmpegLog] = useState("🎬 Estudio de video preparado. Listo para cargar clips.");
  const [videoResult, setVideoResult] = useState(null); // Aquí guardaremos el MP4 generado
  
  // Referencia para mantener vivo el motor FFmpeg
  ...
  useEffect(() => { setSpecificModel(MODEL_VERSIONS[activeModel][0].id); }, [activeModel]);
  ...
  // --- GESTIÓN DE ARCHIVOS ESTUDIO DE VIDEO ---
  ...
  const runFfmpegRender = async () => {
    ...
    setIsRendering(true);
    setVideoResult(null);
    setFfmpegLog("[INFO] Despertando al motor FFmpeg...");

    try {
      const ffmpeg = ffmpegRef.current;

      // 1. Interceptar los logs reales de C++ y mostrarlos en tu pantalla
      ffmpeg.on('log', ({ message }) => {
        setFfmpegLog(prev => `${prev}\n[FFMPEG] ${message}`);
      });

      // 2. Cargar el motor desde los enlaces si no está cargado aún
      if (!ffmpeg.loaded) {
        setFfmpegLog(prev => `${prev}\n[INFO] Descargando Cerebro WASM (esto puede tardar unos segundos)...`);
        await ffmpeg.load({
      ...
      setFfmpegLog(prev => `${prev}\n[INFO] Motor listo. Construyendo sistema de archivos virtual...`);

      // 3. Escribir las imágenes reales en la memoria del navegador
      for (let i = 0; i < videoFiles.length; i++) {
        const fileData = await fetchFile(videoFiles[i].file);
        // Las renombramos a img0.jpg, img1.jpg, etc. para que FFmpeg las entienda en secuencia
        await ffmpeg.writeFile(`img${i}.jpg`, fileData);
      }

      setFfmpegLog(prev => `${prev}\n[INFO] Renderizando Video Faceless (Formato Vertical 1080x1920)...`);

      // 4. Ejecutar el Comando Matemático (Slideshow Vertical a 30fps, 2 segundos por foto)
      await ffmpeg.exec([
        '-framerate', '1/2', // Cada imagen dura 2 segundos
        '-i', 'img%d.jpg',   // Leer todas las imagenes en secuencia
        '-c:v', 'libx264',   // Codec de video universal MP4
        '-r', '30',          // 30 Frames por segundo
        '-pix_fmt', 'yuv420p',
        // Filtro mágico: Escala a 1080p, recorta lo que sobra para que quede vertical perfecto
        '-vf', 'scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920',
        'output.mp4'         // Archivo de salida
      ]);

      setFfmpegLog(prev => `${prev}\n[INFO] ¡Magia completada! Empaquetando archivo final...`);

      // 5. Leer el archivo resultante y crear una URL reproducible
      const data = await ffmpeg.readFile('output.mp4');
      const videoBlob = new Blob([data.buffer], { type: 'video/mp4' });
      const videoUrl = URL.createObjectURL(videoBlob);

      setVideoResult(videoUrl);
      setFfmpegLog(prev => `${prev}\n✅ ¡VIDEO EXPORTADO CON ÉXITO!`);
    ...
    try {
      ...
      if (activeModel === 'gemini') {
        ...
        const res = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({ model: specificModel, max_tokens: 8192, system: systemInstruction, messages: [...claudeHistory, { role: 'user', content: currentContent }] })
        });
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-black text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto pb-48 relative">
          ...
          <div className="p-4 space-y-4">
            {currentMessages.length === 0 && (
              <div className="text-center text-gray-500 mt-10 bg-gray-900 border border-gray-800 p-6 rounded-2xl">
                ...
                <p className="text-sm mt-2">Usa Plan 🗺️ ➔ Think 🤔 ➔ Build 1️⃣ o cambia al Director de Cine 🎬</p>
              </div>
            )}
        ...
        {/* VISTA 2: ESTUDIO DE VIDEO REFORZADO */}
        {activeTab === 'studio' && (
          <div className="p-6 space-y-6">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">Sube imágenes para generar tu video Faceless. El motor FFmpeg.wasm procesará los archivos localmente en tu celular.</p>
            ...
            {/* BOTÓN DE RENDERIZADO MÁGICO */}
            <button onClick={runFfmpegRender} disabled={isRendering || videoFiles.length === 0} className={`w-full font-bold py-4 rounded-xl transition-all shadow-lg flex justify-center items-center gap-2 ${isRendering ? 'bg-amber-600 animate-pulse' : 'bg-gradient-to-r from-red-600 to-amber-600 text-white disabled:opacity-50 disabled:cursor-not-allowed'}`}>
              {isRendering ? "⚙️ Renderizando Lote..." : "🎬 Compilar Lote en Video"}
            </button>

            {/* PANTALLA DEL REPRODUCTOR (Aparece cuando el video está listo) */}
            {videoResult && (
      ...
      {/* NAVEGACIÓN MÓVIL (CUATRO PESTAÑAS) */}
