...
const PERSONAS = {
  ...
  senior_dev: "Eres un Arquitecto de Software Senior. Respuestas estrictamente técnicas. Cero saludos.",
  copywriter: "Eres un Copywriter experto en ventas. Tu objetivo es persuadir y escribir textos que conviertan.",
  sarcastic: "Eres una IA genio pero extremadamente sarcástica. Resuelves el problema perfectamente, pero te burlas sutilmente de los humanos."
};
...
export default function AppUI() {
  ...
  // Selectores
  const [activeModel, setActiveModel] = useState('openai'); // Proveedor (Empresa)
  const [specificModel, setSpecificModel] = useState(MODEL_VERSIONS['openai'][0].id); // Modelo exacto
  const [activePersona, setActivePersona] = useState('default'); // Modo
  ...
  // Actualizar modelo específico cuando se cambia de proveedor
  useEffect(() => {
    setSpecificModel(MODEL_VERSIONS[activeModel][0].id);
  }, [activeModel]);

  // Cargar llaves y CHATS
  useEffect(() => {
    const loadedKeys = {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-black text-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto pb-48 relative">
          ...
          <div className="p-4 space-y-4">
            {currentMessages.length === 0 && (
              <div className="text-center text-gray-500 mt-10 bg-gray-900 border border-gray-800 p-6 rounded-2xl">
                ...
                <p className="font-bold text-gray-300">El Método PTB</p>
                <p className="text-sm mt-2">1. Plan 🗺️ ➔ 2. Think 🤔 ➔ 3. Build 🏗️</p>
              </div>
            )}
            ...
            {currentMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[90%] p-4 rounded-2xl text-sm shadow-md ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-sm' : 'bg-gray-900 text-gray-100 border border-gray-700 rounded-bl-sm w-full'}`}>
                  ...
                  {/* BARRA SUPERIOR PARA MENSAJES DE LA IA (Copiar Todo) */}
                  {msg.role === 'assistant' && (
                    <div className="flex justify-between items-center mb-3 pb-2 border-b border-gray-700/50">
                      ...
                      <button 
                        ...
                        className="text-[10px] uppercase font-bold tracking-wider flex items-center gap-1 text-gray-400 hover:text-white bg-gray-800 px-2 py-1 rounded transition-colors"
                        title="Copiar todo el mensaje"
                      >
                        📋 Copiar Todo
                      </button>
                    </div>
        ...
        {/* BÓVEDA DE APIS (Añadido GoHighLevel) */}
        {activeTab === 'settings' && (
      ...
      {activeTab === 'chat' && (
        <div className="fixed bottom-[70px] left-0 w-full bg-gray-900 border-t border-gray-800 z-10 p-2 flex flex-col gap-2 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
          ...
          {/* TRIPLE SELECTOR: PROVEEDOR | VERSIÓN | MODO */}
          <div className="grid grid-cols-3 gap-1">
            <select value={activeModel} onChange={(e) => setActiveModel(e.target.value)} className="bg-black border border-gray-700 text-[10px] md:text-xs text-blue-400 font-bold rounded-lg p-2 outline-none focus:border-blue-500">
            ...
            <select value={specificModel} onChange={(e) => setSpecificModel(e.target.value)} className="bg-black border border-gray-700 text-[10px] md:text-xs text-green-400 font-bold rounded-lg p-2 outline-none focus:border-green-500">
              {MODEL_VERSIONS[activeModel].map(m => (
                <option key={m.id} value={m.id}>{m.name}</option>
              ))}
            </select>

            <select value={activePersona} onChange={(e) => setActivePersona(e.target.value)} className="bg-black border border-gray-700 text-[10px] md:text-xs text-purple-400 font-bold rounded-lg p-2 outline-none focus:border-purple-500 truncate">
